...
function draw(){
  wrap.innerHTML='';
  if(!groups.length) wrap.innerHTML='<p style="text-align:center;padding:20px">No groups yet. Click Admin to create.</p>';
  ['Daily','Weekly','Monthly'].forEach(t=>{
    ...
    list.forEach(g=>{
      ...
      grid.innerHTML+=`<div class="card"><h3>${g.title}</h3><p>${g.desc}</p><span class="badge">${left} slots left</span><button onclick="join('${g.id}')" ${left<=0?'disabled':''}>${left<=0?'Full':'Join Group'}</button>${isAdmin?`<button onclick="del('${g.id}')" style="background:#eee;color:#800020">Delete Live</button>`:''}</div>`;
    });
  });
}
...
document.getElementById('adminBtn').onclick=()=>{
  ...
  if(p==='lucia123'){ isAdmin=true; document.getElementById('createBtn').style.display='inline-block'; draw(); alert('Admin LIVE mode on!'); }
}
